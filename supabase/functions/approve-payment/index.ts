...
// 
// 환경변수 설정 필요:
// - TOSS_SECRET_KEY: 토스페이먼츠 시크릿 키 (Supabase Secrets에 설정)
// ============================================
...
serve(async (req) => {
  ...
  try {
    ...
    // 필수 파라미터 검증
    if (!paymentKey || !orderId || !amount || !items) {
      return new Response(
        JSON.stringify({
          error: "필수 파라미터가 누락되었습니다.",
          required: ["paymentKey", "orderId", "amount", "items"],
        }),
        {
          status: 400,
    ...
    try {
      ...
      // customer_email이 있으면 customers 테이블에서 customer_id 찾기
      let customerId: number | null = null;
      if (customerEmail) {
        try {
          // customers 테이블에서 이메일로 조회
          ...
          console.warn("고객 정보 조회/생성 중 오류:", error);
        }
      }

      // 주문 생성 (customer_email도 함께 저장)
      const orderData: any = {
      ...
      // customer_email이 있으면 추가
      if (customerEmail) {
        orderData.customer_email = customerEmail;
      }

      // user_id가 있으면 추가 (기존 웹사이트 호환성)
      if (userId) {
